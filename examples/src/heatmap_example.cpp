...
__global__ void test(float *dst, float *src, float alpha, size_t array_size, dh_comms::dh_comms_descriptor *rsrc) {
  ...
  dh_comms::v_submit_address(rsrc, src + idx, 0, __LINE__, 0, __LINE__, dh_comms::memory_access::read,
                             dh_comms::address_space::global, sizeof(float));
  dh_comms::v_submit_address(rsrc, dst + idx, 0, __LINE__, 0, __LINE__, dh_comms::memory_access::write,
                             dh_comms::address_space::global, sizeof(float));
...
void get_options(int argc, char **argv, size_t &array_size, int &threads_per_block, size_t &no_sub_buffers,
                 size_t &sub_buffer_capacity, size_t &page_size, bool &verbose) {
...
int main(int argc, char **argv) {
  ...
  get_options(argc, argv, array_size, blocksize, no_sub_buffers, sub_buffer_capacity, page_size, verbose);
