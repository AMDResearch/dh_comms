...
namespace dh_comms
{
    ...
    //-------------------
    memory_heatmap_v2_t::memory_heatmap_v2_t(size_t page_size, bool verbose)
        : verbose_(verbose),
          page_size_(page_size)
    ...
    memory_heatmap_v2_t::~memory_heatmap_v2_t()
    {
        show();
    }
    ...
    void memory_heatmap_v2_t::merge_state(message_handler_base &other)
    {
        memory_heatmap_v2_t &other_mh = dynamic_cast<memory_heatmap_v2_t &>(other);
