...
namespace
{
    std::vector<dh_comms::lane_header_t> init_lane_headers(const char *message_p, const dh_comms::wave_header_t &wave_header)
    {
    ...
    {
        const uint32_t *data_p = (const uint32_t *)(message_p +
                                                    sizeof(dh_comms::wave_header_t) +
                                                    wave_header.has_lane_headers * wave_header.active_lane_count * sizeof(dh_comms::lane_header_t));
        size_t data_size = wave_header.data_size - wave_header.has_lane_headers * wave_header.active_lane_count * sizeof(dh_comms::lane_header_t);
